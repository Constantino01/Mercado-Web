...
export default function Pedidos() {
  ...
  const carregarPedidos = async () => {
    try {
      ...
      // A NOSSA VACINA: Se for um array direto usa-o, se vier dentro de .data, extrai-o!
      const arrayPedidos = Array.isArray(response) ? response : (response?.data || []);
      
      const ordenadas = arrayPedidos.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-gray-600 whitespace-nowrap">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              /* SKELETON LOADING PARA A TABELA DE ENCOMENDAS */
                    ...
                    <div key={produto.id} className="bg-white border border-gray-100 rounded-xl p-3 flex items-center gap-4 shadow-sm">
                      <div className="bg-gray-50 w-14 h-14 rounded-lg flex items-center justify-center text-2xl shrink-0 overflow-hidden border border-gray-100">
                        ...
                        {produto.product_image ? (
                           <img src={produto.product_image} alt={produto.product_name} className="w-full h-full object-contain p-1"/>
                        ) : '📦'}
                      </div>
