...
export default function DescontosRecycle() {
  ...
  const carregarLixeira = async () => {
    try {
      const data = await apiCRUD.read('/descontos-lixeira');
      setDescontos(Array.isArray(data) ? data : (data.data || []));
    } catch (error) {
      console.error('Erro ao carregar a lixeira de descontos', error);
    } finally {
      setLoading(false);
    }
  };

  const restaurarDesconto = async (id) => {
    try {
      await apiCRUD.create(`/descontos/${id}/restore`, {});
      setDescontos(descontos.filter(d => d.id !== id));
      alert('Desconto restaurado com sucesso!');
    } catch (error) {
      alert(error.message || 'Não foi possível restaurar o desconto.');
    }
  };

  const apagarDefinitivamente = async (id) => {
    if (window.confirm('ATENÇÃO: Ação irreversível! Deseja mesmo apagar este desconto permanentemente?')) {
      try {
        await apiCRUD.delete(`/descontos/${id}/force`);
        setDescontos(descontos.filter(d => d.id !== id));
      } catch (error) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-red-50/30">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Reciclagem de Descontos</h1>
        </div>
      </div>
      ...
      <div className="p-6 bg-gray-50/50">
        <div className="relative w-full sm:w-72">
          ...
          <input 
            type="text" 
            placeholder="Pesquisar desconto apagado..." 
            value={pesquisa}
            ...
            {loading ? (
              ...
              descontosFiltrados.map((d) => (
                <tr key={d.id} className="even:bg-gray-50 hover:bg-gray-100 transition-colors">
                  <td className="px-6 py-4 font-bold text-gray-400 line-through">{d.discount_name}</td>
                  ...
                  <td className="px-6 py-4 text-gray-500">
                    {d.deleted_at ? new Date(d.deleted_at).toLocaleDateString('pt-PT') : '-'}
                  </td>
                  <td className="px-6 py-4 flex justify-end gap-3">
                    <button onClick={() => restaurarDesconto(d.id)} className="text-gray-400 hover:text-green-600 transition-colors" title="Restaurar">
                    ...
                    <button onClick={() => apagarDefinitivamente(d.id)} className="text-gray-400 hover:text-red-700 transition-colors" title="Apagar Definitivamente">
